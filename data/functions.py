...
def json_to_lst(j_x, j_y):
    name=[]
    for i in j_x:
        name.append(i[j_y])
    return name
    
def clean_count_dic(path):
    with open(path) as x:
        data = json.load(x)
    lst=[]
    for i in data:
    ...
    lst_clean= [a for i in lst for a in i]

    dictionary = {}
    dictionary.__setitem__('places', len(lst))
    for item in lst_clean:
        dictionary[item] = dictionary.get(item, 0) + 1

    
    return print(dictionary)

def clean_count_2(path):
    with open(path) as x:
        data = json.load(x)
    lst=[]
